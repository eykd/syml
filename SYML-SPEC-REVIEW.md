...
| D31 | *(2026-09-25)* `EncodingError` says "Invalid UTF-8 (byte 0xe9); save the file as UTF-8", naming the first offending byte in lowercase hex, instead of "Invalid encoding". *Refined `syml-cjk2.21`:* that text is now reserved for a genuinely-UTF-8 decode failure (`codecs.lookup(err.encoding).name == 'utf-8'`); a caller-supplied text stream decoded with another codec instead gets "Invalid {codec} (byte 0xe9)", naming that codec and dropping the UTF-8-specific advice, since re-saving as UTF-8 would not fix a codec mismatch. *Refined `syml-cjk2.26`:* `err.encoding` is unreliable for that codec name — every charmap-based codec (`cp1252`, `cp437`, the `latin-*` family, ...) reports the literal `'charmap'` there, not the codec the caller chose. `load()` now passes the stream's own `getattr(file_obj, 'encoding', None)` into the `EncodingError` builder, preferred over `err.encoding`, which remains the fallback for bytes input (no stream to ask). *Refined `syml-cjk2.28`:* that `.26` change introduced two regressions, both now fixed. First, `utf-8-sig` (a stream's own codec name for UTF-8-with-BOM) was compared against the literal string `'utf-8'` and so lost the UTF-8-specific advice; the comparison now checks `codecs.lookup(named_encoding).name in ('utf-8', 'utf-8-sig')`. Second, a stream `.encoding` that is not a `str`, or is a `str` naming an unknown codec, was passed straight into `codecs.lookup`, leaking a bare `TypeError`/`LookupError` instead of `EncodingError`; `stream_encoding` is now used only when it is a `str` that `codecs.lookup` accepts, and `err.encoding` is the fallback in both failure shapes, not only for bytes input. | Leave it (`syml-cjk2.15` option B). Message text only. |
...
| D35 | *(2026-09-25)* Amends D26: hint (c)'s key alternative excludes `://` immediately after the colon — `^[a-z][a-z0-9_-]*:(?!//)\S`, on both the failing line and the look-back — so a legitimate URL value (`http://example.com`, and §8's own `url: https://example.com:8080/path` example) never earns "a key or list marker needs a space after it"; that hint would tell an author to turn a valid value into a key (`syml-cjk2.20`). | Narrow the exclusion to only `http://`/`https://` (rejected: any `scheme://` value, e.g. `ftp://`, `git://`, hits the same false hint; a general `://` exclusion covers them all without naming schemes). |

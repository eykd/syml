...
| a Green worker splits its task into a discovered sub-chain | its RED test stays red for every later tick, so every non-RED task then fails pre-commit's full-suite gate. Bridge it: the next Green marks the still-red cases `pytest.mark.xfail(strict=True, reason=<chain id>)` in the same commit, and the chain's Green task bodies say to remove each mark as its case passes. No mark may remain when the chain closes |
| two REDs from different chains are on the branch at once | every Green deadlocks (each fails the gate on the other's red test). Prevent it by bumping a RED's own Green to p1 the moment the RED closes so prep serves it next; once deadlocked, give one worker both Greens to land in a single commit and close both tasks |
| worker reports `FAILED <id>: filed …`                  | after `verify.sh … NONE`, run `br show <id>`: workers file Red/Green/Refactor sub-chains reliably but wire the verify leaf's `blocks` edge to the chain's last leaf only about half the time, and occasionally file a duplicate. Add the missing `br dep add <verify-leaf> <last-leaf>`, run `br dep cycles`, and close exact duplicates before the next prep |
| `br create` / `br update` blocked by the Bash guard hook | the PreToolUse guard checks the orchestrator's and reviewers' own commands, so a task body or probe string naming a blocked flag (hook-skip, force-push, recursive rm) trips it. Write bodies to a scratch file and pass `--description-file`; keep probe strings in files, never on the command line |

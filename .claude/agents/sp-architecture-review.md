---
name: sp-architecture-review
description: Architecture compliance review of all branch changes (base..HEAD). Runs /clean-architecture-validator skill, creates remediation tasks or appends implementation constraints. Writes .sp-harden-findings.json so orchestrators (/sp:08-harden) can count p1/p2 findings.
...
## Implementation Constraints (from [sp:architecture-review])
...
br create "Remediate: <short finding title>" -p <1|2|3> --parent <epic-id> \
  --description "**Review**: [sp:architecture-review]\n**Range**: <base>..HEAD\n**Files**: <file paths>\n\n**Finding**:\n<what's wrong>\n\n**Fix suggestion**:\n<concrete steps>\n\n**Acceptance**:\n- <verifiable criteria>" --json
````
...
7. Cross-reference with Prior Learnings:
   ...
   - Search `.specify/solutions/clean-architecture/` and `.specify/solutions/python-typing/` for solutions related to current findings
...
8. Write structured findings file for orchestrators:
   ...
   ```bash
   .venv/bin/python -m tools.harden_fingerprint --phase architecture-review < FEATURE_DIR/.sp-findings.json
   ```
   ...
   ```bash
   echo '[]' | .venv/bin/python -m tools.harden_fingerprint --phase architecture-review
   ```

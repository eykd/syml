...
7. Cross-reference with Prior Learnings:
   ...
   - Search `.specify/solutions/pytest-coverage/`, `.specify/solutions/python-typing/`, and `.specify/solutions/clean-architecture/` for solutions related to current findings
   ...
   - Example addition to task description: `\n\n**Prior Learning**: See .specify/solutions/pytest-coverage/{slug}.md for a previous solution to this pattern.`
...
8. Write structured findings file for orchestrators:
   ...
   ```bash
   .venv/bin/python -m tools.harden_fingerprint --phase code-quality-review < FEATURE_DIR/.sp-findings.json
   ```
   ...
   ```bash
   echo '[]' | .venv/bin/python -m tools.harden_fingerprint --phase code-quality-review
   ```

...
**Version:** 1.1
**Status:** Draft specification. No implementation fully conforms yet. The
`syml` Python package (currently v0.6.2) predates this specification and
diverges from it in the ways enumerated in §14 (version history) —
including returning `None` rather than `""` for empty and comment-only
documents (§7.4), not decoding quoted strings (§4.7), and accepting tabs
in indentation (§4.2, §8.4). Do not treat v0.6.2 as a conformance oracle;
§2-§13 are authoritative.
...
v1.1 it does not yet conform to this specification (see the status line at
the top of this document and §14); a conforming release is pending.

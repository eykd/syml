...
| `test_parsers.py::TestSymlParser::test_it_should_parse_a_nested_list_with_mapping` (done: rewritten as `test_it_should_reject_an_indentless_list_under_a_key`, asserting `OutOfContextNodeError`; `test_it_should_parse_comments_and_blanks`'s own indentless list under `- foo:` was re-indented in the same pass so it keeps pinning only the comment behavior it owns) | indentless list under `- foo:` (D25) | strict depth |

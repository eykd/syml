...
- **Unrepresentable set** (shrunk): the values `dumps` refuses, now limited to first lines that lex as structure at a list or block position, first lines beginning with a space (non-root), a tab in any line's leading whitespace, whitespace-only lines, values beginning or ending with a blank line, a later line opening with a chain of more than 32 `- ` markers, control characters other than tab, and keys outside the key pattern.

...
their text, and refuses only FR-006's residual families. Two families of values that
`loads` can return stay refused: a multi-line mapping value whose first line is
shaped like structure (it has an inline-first spelling `dumps` declines to use), and
...
4. **Given** `CHANGELOG.md`'s 1.0.0 entry, **When** read by a 0.6.2 user, **Then** items 4, 7, 10, 12, 13, 16, and 17 describe what master does, and new items state that comments are gone (warning that in a third-party file a `#` line after a container's first entry now raises, while a `#` first line of the file or of a block silently makes that file or block one string), keys are `[a-z][a-z0-9_-]*`, values are text throughout, blank lines inside values are kept, indentless sequences are rejected, and a tab after a marker is separator whitespace.
...
- **FR-006** (R6; `syml-xreq.15`, `.22`, `.2`, `.6`): `dumps` MUST write every value FR-003 to FR-005 make loadable: paragraph breaks as blank lines in block form, later lines of any shape, and a root scalar with leading spaces in block form at column 0. Whether a line lexes as structure MUST be judged on the line's own characters without the §9.0 BOM strip, so a BOM-led line is content. The unrepresentable set of §11.2.1 MUST shrink to: a value whose first line lexes as structure at a list position (rule B) or at a block position (rule C); a non-root value whose first line begins with a space; a value whose first line begins with a tab; a value that begins or ends with a blank line; a value with a line that is whitespace-only but not empty (it would read back as an empty line); a value with a tab anywhere in a line's leading run of spaces and tabs (the §9.0 tab scan), on any line; a value with a later line whose per-line lexing exceeds the parser's recursion depth (a long `- - - …` chain, which `loads` cannot read back although FR-003 makes it text); and control characters per rule D. The residual list was confirmed during planning (research.md R-05).
...
- **Unrepresentable set** (shrunk): the values `dumps` refuses, now limited to first lines that lex as structure at a list or block position, first lines beginning with a space (non-root), a tab in any line's leading whitespace, whitespace-only lines, values beginning or ending with a blank line, a later line too deeply `- `-chained for the per-line lex to read back, control characters other than tab, and keys outside the key pattern.
